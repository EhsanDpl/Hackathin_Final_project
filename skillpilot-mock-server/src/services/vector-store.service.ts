...
export class VectorStoreService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    // Only initialize if vector store is enabled via environment variable
    if (!process.env.ENABLE_VECTOR_STORE || process.env.ENABLE_VECTOR_STORE !== 'true') {
      return;
    }
    ...
    try {
      ...
      console.warn('Vector store initialization failed, document chat will not be available:', error.message);
    }
  }
  ...
  async addDocuments(documents: Document[]): Promise<void> {
    if (!this.pgvectorStore) {
      throw new Error('Vector store is not initialized. Set ENABLE_VECTOR_STORE=true to enable.');
    }
